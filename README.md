...
* **Abierto-Cerrado (OCP):** El sistema es extensible. Si se requiere un nuevo tipo de análisis se puede implementar una nueva clase que cumpla con `AnalysisCommand` sin modificar el código existente en `NumericInterval` o en otros comandos.

**Extras:**
* **Paquete Command:** Se utiliza para encapsular las lógicas complejas de "Contar IDs válidos" y "Calcular Cobertura", aligerando el paquete controler.
* **Java Records:** Uso de `NumericInterval` para definir objetos de valor inmutables y comparables.
* **Algoritmo de Fusión de Intervalos:** Implementación en `CalculateCoverageCommand` utilizando Streams y `reduce` para ordenar y combinar rangos solapados en una sola pasada.
* **Streams y Programación Funcional:** Uso de `stream()`, `filter`, `map`, `reduce` y `parallelStream` para procesar listas de datos de manera declarativa y eficiente.
